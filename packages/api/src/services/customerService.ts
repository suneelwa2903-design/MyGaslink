...
const customerInclude = {
  ...
  // 2026-07-21 Mini-Operator opening state: allowlist rows exposed as
  // `allowedCylinderTypeIds` after mapCustomer (see utils/mappers.ts).
  // Order form + delivery picker consume the flat id list; the joined
  // typeName is retained for display in the Customer detail modal.
...
// ─── Mini-Operator opening state seed (2026-07-21) ─────────────────────────
//
// One-shot "customer onboarding with opening ledger state" for mini-op
// (reseller) tenants. Wraps createCustomer and ALL seed steps in one
// atomic transaction:
//
//   1. base customer row
//   2. per-customer cylinder-type allowlist (order picker restriction)
//   3. seeded empties balance per cylinder type (dashboard "with clients")
//   4. optional ₹ opening balance → synthetic OB invoice + item + ledger
//   5. audit marker on customers.opening_state_seeded_at
//
// Any sub-step throws → the whole transaction rolls back → no half-
// created customer. Reads reconcile automatically:
//   - individual + group ledger emit "Opening Balance b/f" row
//   - overdue counts OB debt past credit window (Fix C, 2026-06-11)
//   - dashboard Total Outstanding includes OB invoice.outstanding_amount
//   - customer inventory dashboard reads with_customer_qty (state-current)
//   - order form / delivery picker filters by the allowlist
//
// Gate: tenant MUST have accountType='mini_operator'. Regular
// distributors get 400 — they use the CSV importer at Settings →
// Onboarding.
interface OpeningStateSeed {
...
export async function createCustomerWithOpeningState(
  ...
  seeded: {
    ...
    allowedCylinderTypeCount: number;
  ...
  // Fast path: no opening state → delegate to the existing createCustomer
  // (preserves warnings + include shape, no transaction overhead).
  if (!openingState) {
  ...
  // and is active + not deleted. We do this up front (outside the tx) so
  // the transaction body is small and predictable.
  const referencedTypeIds = Array.from(new Set([
    ...(openingState.allowedCylinderTypeIds ?? []),
    ...(openingState.empties ?? []).map((e) => e.cylinderTypeId),
  ]));
  ...
  // GSTIN soft-warning check runs the same way createCustomer does — we
  // deliberately DON'T inline createCustomer here because we need every
  // step under one $transaction and createCustomer isn't tx-friendly.
  const warnings: string[] = [];
  ...
  const asOf = openingState.openingBalance?.asOfDate;
  const asOfDate = asOf && /^\d{4}-\d{2}-\d{2}$/.test(asOf) ? new Date(asOf) : new Date();

  const result = await prisma.$transaction(async (tx) => {
    // 1. base customer row + contacts + discounts (same shape as createCustomer)
    ...
    // 2. Allowed cylinder types (order picker restriction). Duplicates
    // impossible via the composite unique — but we dedupe defensively.
    const allowedIds = Array.from(new Set(openingState.allowedCylinderTypeIds ?? []));
    if (allowedIds.length > 0) {
      await tx.customerAllowedCylinderType.createMany({
        data: allowedIds.map((cylinderTypeId) => ({
    ...
    // 3. Seeded empties balances (state-current view). One row per type
    // with qty > 0; qty=0 rows are skipped to keep the table lean.
    const emptiesRows = (openingState.empties ?? []).filter((e) => e.qty > 0);
    ...
    // 4. Optional ₹ opening balance — synthetic OB invoice + item +
    // ledger entry. Mirrors importOpeningBalances but scoped to one row
    // and inline in this transaction. Status='overdue' matches the
    // existing importer's convention (Fix C 2026-06-11 makes overdue
    // computation include past-credit-window OB debt).
    let openingInvoiceId: string | null = null;
    if (openingState.openingBalance && openingState.openingBalance.amount > 0) {
      const ob = openingState.openingBalance;
      const invoiceNumber = `OB-${customer.id.slice(0, 8)}-${asOfDate.toISOString().split('T')[0]}-${Math.random().toString(36).slice(2, 6)}`;
      const invoice = await tx.invoice.create({
        data: {
          ...
          notes: ob.notes?.trim() || 'Opening balance recorded during customer onboarding',
          issuedBy: userId,
        },
      });
    ...
    return {
      ...
      seeded: {
        ...
        allowedCylinderTypeCount: allowedIds.length,
