...
export default function InvoicesPage() {
  ...
  // WI-055: the cylinder-types query previously fed the CN/DN items grid.
  // The amount-based redesign drops that dependency entirely — no need
  // to fetch the list from this page anymore.
...
function InvoiceDetailModal({
  ...
  // NIC IRN/EWB cancellation window — NIC rejects cancels >24h after IRN
  // ack. We gate the cancel button by ack age so users don't trigger a
  // guaranteed-fail cancel that lands the invoice in cancel_failed limbo.
  // Retry stays accessible regardless of window when status is cancel_failed
  // (per anti-pattern #15 — cancel-retry is safe and idempotent at NIC).
  // EWB date isn't on the Invoice type; ackDate is a 99%-accurate proxy
  // since EWB is generated seconds after IRN in our flow.
...
// GROUP-7S: CancelGstModal extracted to packages/web/src/components/CancelGstModal.tsx
// with a NIC reason-code dropdown (required by the server as of GROUP-7S).

// ─── Credit Note Modal ──────────────────────────────────────────────────────
//
// WI-055: amount-based form (Reason + Amount + Note). Mirrors the modal in
// BillingPaymentsPage.tsx so both entry points stay in sync. See spec at
// .session/specs/WI-055-credit-note-modal-redesign.md.
...
// ─── Debit Note Modal ───────────────────────────────────────────────────────
//
// WI-055: amount-based. No upper bound — surcharges legitimately exceed
// the invoice total.
