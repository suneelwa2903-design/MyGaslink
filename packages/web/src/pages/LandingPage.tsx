...
import {
  ...
  HiOutlineXMark, HiOutlineCheckCircle, HiOutlineStar, HiOutlineBolt,
...
function Counter({ target, suffix = '' }: { target: number; suffix?: string }) {
  ...
  useEffect(() => {
    if (!inView) return;
    let s = 0;
    const step = (t: number) => { if (!s) s = t; const p = Math.min((t - s) / 2000, 1); setCount(Math.floor((1 - Math.pow(1 - p, 3)) * target)); if (p < 1) requestAnimationFrame(step); };
...
export default function LandingPage() {
  ...
  const features = [
    ...
    { icon: HiOutlineCube, title: 'Smart Inventory', desc: 'Event-sourced cylinder tracking — know where every cylinder is, right now.', bullets: ['Real-time full & empty counts', 'Incoming/outgoing document tracking', 'Low-stock & critical alerts', 'Daily locks & reconciliation'] },
    ...
    { icon: HiOutlineTruck, title: 'Fleet & Delivery', desc: 'Driver-vehicle assignments, trip pipelines, and end-of-day reconciliation.', bullets: ['Smart driver-vehicle pairing', 'Trip status pipeline', 'Vehicle inventory tracking', 'Delivery proof & disputes'] },
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950 text-[#1e3a5f] dark:text-white transition-colors duration-300 relative">
      ...
      {/* ─── Hero ──────────────────────────────────────────────────────── */}
      <section className="relative pt-28 pb-20 lg:pt-36 lg:pb-28 px-6 lg:px-12 xl:px-20 text-[#1e3a5f] dark:text-white">
        <div className="max-w-5xl mx-auto text-center">
          <motion.span initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="inline-flex items-center gap-2 px-4 py-2 bg-flame-500/10 border border-flame-500/20 rounded-full text-sm font-semibold text-flame-500 mb-8"><HiOutlineBolt className="h-4 w-4" />Trusted by 500+ LPG distributors across India</motion.span>
          <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .1 }} className="text-5xl sm:text-6xl lg:text-[4.5rem] font-extrabold leading-[1.1] tracking-tight mb-8 text-[#1e3a5f] dark:text-white">Every Cylinder. Every Invoice.<br className="hidden sm:block" /><span className="text-flame-500">Every Rupee. Tracked.</span></motion.h1>
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .2 }} className="text-lg lg:text-xl text-slate-500 dark:text-slate-400 max-w-3xl mx-auto mb-10 leading-relaxed">Complete operations platform for LPG distributors. Orders, inventory, GST invoicing, fleet management, payments, and analytics — all in one place.</motion.p>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .3 }} className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-5">
            <button onClick={() => scrollTo('contact')} className="px-8 py-4 bg-flame-500 hover:bg-flame-600 text-white font-semibold rounded-xl transition-colors flex items-center gap-2 text-base">Claim 3 Months Free <HiOutlineArrowRight className="h-5 w-5" /></button>
          ...
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: .5 }} className="text-sm text-slate-400">No credit card · 15 min setup · Cancel anytime</motion.p>
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .6 }} className="mt-16 grid grid-cols-2 lg:grid-cols-4 gap-px bg-slate-100 dark:bg-white/10 rounded-2xl overflow-hidden">
            {[{ v: 500, s: '+', l: 'Distributors' }, { v: 10, s: 'L+', l: 'Cylinders Tracked' }, { v: 99, s: '.9%', l: 'Uptime' }, { v: 2, s: 'Cr+', l: 'Savings Generated' }].map((s, i) => (
              <div key={i} className="bg-slate-50 dark:bg-white/5 backdrop-blur-sm p-7 text-center">
                <p className="text-3xl lg:text-4xl font-extrabold mb-1">{s.l === 'Uptime' ? '99.9%' : s.l === 'Savings Generated' ? <span>₹<Counter target={s.v} suffix={s.s} /></span> : <Counter target={s.v} suffix={s.s} />}</p>
                <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">{s.l}</p>
              </div>
            ))}
          </motion.div>
        </div>
      ...
      {/* ─── Testimonials ──────────────────────────────────────────────── */}
      <section id="testimonials" className="py-24 px-6 lg:px-12 xl:px-20 bg-slate-50/80 dark:bg-slate-900/40">
        <div className="max-w-6xl mx-auto">
          <Reveal><div className="text-center mb-16"><span className="text-xs font-bold text-flame-500 uppercase tracking-widest">Testimonials</span><h2 className="text-4xl lg:text-5xl font-extrabold mt-3">Loved by Distributors</h2></div></Reveal>
          <div className="grid md:grid-cols-3 gap-6">
            {[{ n: 'Rajesh Kumar', r: 'HP Gas, Hyderabad', t: 'Cylinder losses dropped 80% in month one. The accountability module alone pays for itself 10x.' },
              { n: 'Priya Sharma', r: 'Bharat Gas, Bangalore', t: 'GST invoicing took 3 days. Now one click. Our CA sent us a thank-you note.' },
              { n: 'Amit Patel', r: 'Indane Gas, Mumbai', t: 'Real-time inventory changed everything. We never run out and customers love the portal.' }].map((t, i) => (
              <Reveal key={i} delay={i * .1}><div className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-8 h-full flex flex-col hover:border-flame-300 dark:hover:border-flame-500/30 transition-colors">
                <div className="flex gap-1 mb-5">{[1,2,3,4,5].map(s => <HiOutlineStar key={s} className="h-5 w-5 text-flame-400 fill-flame-400" />)}</div>
                <p className="text-slate-600 dark:text-slate-300 mb-6 flex-1 leading-relaxed">"{t.t}"</p>
                <div className="flex items-center gap-3"><div className="h-10 w-10 rounded-full bg-flame-100 dark:bg-flame-500/20 flex items-center justify-center text-flame-600 dark:text-flame-400 font-bold text-sm">{t.n.split(' ').map(x => x[0]).join('')}</div><div><p className="font-semibold text-sm">{t.n}</p><p className="text-xs text-slate-500">{t.r}</p></div></div>
              </div></Reveal>
            ))}
          </div>
        </div>
