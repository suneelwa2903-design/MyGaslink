...
- **Submission `attachmentUrl` is validated against the authenticated distributor's S3 prefix** before being persisted. See [lib/s3.ts isOwnedPaymentAttachmentUrl](packages/api/src/lib/s3.ts) — defends against a client attempting to claim attachments uploaded under another tenant's path.
