...
| 22 | 2026-05-20 | GST > Cancel | IRN cancel returned SESSION_EXPIRED when order-cancel runs EWB+IRN cancel back-to-back — double clearTokenCache forced 2 auth calls <1 s apart; NIC rejected second token with 1004 | ✅ Fixed (WI-086 640d348 — clearTokenCache moved to cancelOrder orchestrator, single eviction before both calls; standalone retry routes handle their own eviction) |
...
| 27 | 2026-05-21 | Fleet > Vehicle Return | "Mark as Returned" button missing — vehicle stuck at `idle` after dispatch. Two causes: (a) `preflightAddToTrip` never set vehicle.status='dispatched' (only `preflightDispatch` did); (b) GST integration tests blanket-reset ALL dist-002 vehicles to `idle` in teardown (`updateMany where distributorId`), corrupting live dispatch state on the shared dev DB. | ✅ Fixed (WI-090 FIX 1 — add-to-trip now sets vehicle dispatched; FIX 2 — gst-preflight/gst-trip-sheet/gst-dispatch-trip tests use dedicated test vehicles, resets scoped by vehicleNumber). Live verified: add-to-trip → dispatched; vehicle survives a full `pnpm test` run still dispatched. |
| 28 | 2026-05-21 | Reports > Trip Sheet PDF | Order numbers rendered without the hyphen ("ORDMPFG…") and cramped. Order# column inner width was 82pt; "ORD-XXXXXXXXXX" is ~90-92pt at Helvetica 9pt, so PDFKit wrapped at the hyphen (its only break point) and discarded it. `ellipsis` never fired because no `height` was passed to `doc.text` (unbounded height = wrap, not truncate). | ✅ Fixed (WI-090 FIX 3 — Order# 90→102 / Customer 115→103 keeping total 515; added height=ROW_H-2 to every row cell). Live verified: all order numbers render on ONE line (10.4pt) vs wrapped (20.8pt) before. |
...
| 2026-05-21 (WI-090 pt2) | **Three fixes in one pass.** FIX 1: `preflightAddToTrip` now sets vehicle.status='dispatched' on success (was only set by `preflightDispatch`) — so the Fleet "Mark as Returned" button appears for add-to-trip dispatches. FIX 2: eliminated test→live vehicle-status contamination — `gst-preflight`, `gst-trip-sheet`, `gst-dispatch-trip` tests now dispatch DEDICATED test vehicles (helpers.getOrCreateTestVehicle) and scope teardown resets by vehicleNumber instead of blanket `where:{distributorId}`. FIX 3: trip sheet Order# column 90→102pt (Customer 115→103, total still 515) + height bound on row cells so order numbers render on one line with the hyphen instead of wrapping. 484/484 tests pass. Live verified all 4: (1) add-to-trip → TS09 dispatched; (2) TS09 stayed dispatched through a full `pnpm test` run; (3) order numbers one-line (10.4pt vs 20.8pt wrapped); real PDF at packages/api/trip-sheet-WI090-verify.pdf. Bugs #27, #28. NOT yet committed — pending merge approval. | Commit + merge WI-090 pt2; then Phase 1/2 testing |
